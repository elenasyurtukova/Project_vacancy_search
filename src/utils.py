...
def sorted_vacancies(vacancies: list[dict], flag: int) -> list[dict]:
    ...
    if flag == 1:
        ...
        return sorted(vacancies, key=lambda vacancy: int(vacancy['salary_to']), reverse=True)
    return vacancies


def filtered_vacancies_by_words(vacancies: list[dict], filter_words: list)-> list[dict]:
...
def get_vacancies_by_salary(vacancies: list[dict], salary_range: list):
    ranged_vacancies = []
    for vacancy in vacancies:
        if vacancy['salary_from'] >= int(salary_range[0]) and vacancy['salary_to'] <= int(salary_range[1]):
