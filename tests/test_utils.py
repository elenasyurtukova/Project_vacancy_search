import pytest
from src.utils import filtered_vacancies_rur, sorted_vacancies, filtered_vacancies_by_words, get_vacancies_by_salary

def test_filtered_vacancies_rur(vacancies_for_rur):
    result = filtered_vacancies_rur(vacancies_for_rur)
    assert result == [{'name': 'Python-разработчик', 'url': 'https://api.hh.ru/vacancies/120331701?host=hh.ru',
...
def test_sorted_vacancies(vacancies_for_rur):
    assert sorted_vacancies(vacancies_for_rur, flag=1) == [
...
def test_sorted_vacancies_2(vacancies_for_rur):
    assert sorted_vacancies(vacancies_for_rur, flag=2) == [
        {'name': 'Аналитик данных', 'url': 'https://api.hh.ru/vacancies/120353943?host=hh.ru',
         'salary_from': 4000000, 'salary_to': 7000000, 'salary_currency': 'UZS',
         'description': 'Опыт работы с BI-системами (Power BI, Tableau, Metabase и др.)'},
